# Closure test with ROOT dataset, Chapter 5 Thesis
# Checkpoint iwpc_logs/di_electron/version_71/checkpoints/epoch=35-step=153864.ckpt
# Dataset /Users/albaburgosmondejar/Desktop/Input2/hhml_v2_2lSC_QMisID_Vjets_Zee.root
...
branches = branches[int(0.1 * n_total):]

mask_window = (
   ( branches["m_l1l2"] > 81e3) & (branches["m_l1l2"] < 101e3) &
    (branches["l1_pt"] > 25e3) & (branches["l2_pt"] > 25e3) &
    (branches["l1_pt"] < 200e3) & (branches["l2_pt"] < 200e3)
)
# mask_pos_w  = branches["weight"] > 0
mask_sc = np.asarray((branches["same_charge"] == 1) )
mask_oc = np.asarray((branches["same_charge"] == 0)  )

weights_all = np.asarray(branches["weight"], float)
sc_weights  = weights_all[mask_sc]
oc_weights  = weights_all[mask_oc]
N_oc = mask_oc.sum()
N_sc = mask_sc.sum()

# phi1 = 0, phi2 = pi (arbitrary)
oc = np.stack([
    np.asarray(branches["l1_pt" ][mask_oc], float),
    np.asarray(branches["l1_eta"][mask_oc], float),
    np.zeros(N_oc),
    np.asarray(branches["l2_pt" ][mask_oc], float),
    np.asarray(branches["l2_eta"][mask_oc], float),
    np.full(N_oc, np.pi),
], axis=1)

sc = np.stack([
    np.asarray(branches["l1_pt" ][mask_sc], float),
    np.asarray(branches["l1_eta"][mask_sc], float),
    np.zeros(N_sc),
    np.asarray(branches["l2_pt" ][mask_sc], float),
    np.asarray(branches["l2_eta"][mask_sc], float),
    np.full(N_sc, np.pi),
], axis=1)


def plotting(oc, sc, oc_weights, sc_weights, batch_size=50_000):

    q_base = torch.from_numpy(oc.astype(np.float32))
    N = q_base.shape[0]
    ...
    with torch.no_grad():
        for start in range(0, N, batch_size):
            qb = q_base[start:start + batch_size]
            ow = oc_weights[start:start + batch_size]

            exact_iter, cut_pass_lp = trainer.exact_kernel.outcome_with_log_prob_iter_and_cut_pass_log_prob(qb)
            exact_iter = list(exact_iter)

            flip_prob_raw_all.append(cut_pass_lp.exp().cpu().numpy() * ow)

            first_outcome,  first_lp  = exact_iter[0]
            second_outcome, second_lp = exact_iter[1]
            use_first = (first_lp > second_lp).unsqueeze(-1)
            outcome_cond = torch.where(
                use_first,
                first_outcome.unsqueeze(0).expand(qb.shape[0], -1),
                second_outcome.unsqueeze(0).expand(qb.shape[0], -1),
            )
            smear_all.append(
                (qb + trainer.sampled_kernel.draw(torch.cat([outcome_cond, qb], dim=1))).cpu().numpy()
            )

            for k, (exact_outcome, _) in enumerate(exact_iter):
                rep = exact_outcome.repeat((qb.shape[0], 1))
                sim = qb + trainer.sampled_kernel.draw(torch.cat([rep, qb], dim=1))
                simulated_all[k].append(sim.cpu().numpy())
                mw = trainer.log_p_over_q_model(torch.cat([sim, rep], dim=1)).exp().cpu().numpy().squeeze(-1)
                model_weights_all[k].append(mw * ow)

    flip_prob_raw = np.concatenate(flip_prob_raw_all)
    flip_prob     = flip_prob_raw * sc_weights.sum() / flip_prob_raw.sum()
    smear         = np.concatenate(smear_all)
    n_outcomes    = len(simulated_all)
    simulated     = sum(np.concatenate(simulated_all[k]) for k in range(n_outcomes)) / n_outcomes
    model_weight  = np.concatenate([np.concatenate(model_weights_all[k]) for k in range(n_outcomes)])
    ...
    for oc_vals, nn_vals, sc_vals, bins, xlabel in [
            (oc[:, 0],        smear[:, 0], sc[:, 0], np.linspace(30e3, 200e3, 100), 'l1_pt [MeV]'),
            ((oc[:, 1]),   smear[:, 1], sc[:, 1], np.linspace(-2.5, 2.5, 100), 'l1_eta'),
            (oc[:, 3],        smear[:, 3], sc[:, 3], np.linspace(30e3, 200e3, 100), 'l2_pt [MeV]'),
            (oc[:, 4],   smear[:, 4], sc[:, 4], np.linspace(-2.5, 2.5, 100), 'l2_eta'),
            (inv_mass(oc),    inv_mass(smear), inv_mass(sc), np.linspace(81e3, 101e3,100), 'm_l1l2 [MeV]'),
        ]:
        ...
        ax_bot.set_ylabel(r'$\frac{w \cdot \mathrm{OC} - \mathrm{SC}}{\mathrm{SC}}$')
        ax_bot.set_ylim(-1, 1)
...
plotting(oc, sc, oc_weights, sc_weights)