...
class KappaLightning(L.LightningModule, Visualisable):
    ...
    def _shared_step(self, batch, stage: str):
        ...
        y = y.view_as(logits1)
        losses = self.criterion(f, y) 
        loss = (losses * w.abs()).sum() / w.abs().sum()
...
if __name__ == "__main__":
    ...
    path = "/Users/albaburgosmondejar/Desktop/Dataset5/"

    dm = PandasDirDataModule(
        dataset_dir=path,
        feature_cols=["l1_q_over_pt", "l2_q_over_pt", "l1_eta", "l2_eta", "l1_phi", "l2_phi"],
        target_cols=["label"],
        weight_col="weight",
        split=0.6,
    ...
    print('length is', len(val_loader))
    ...
    trainer = L.Trainer(
        ...
        logger=TensorBoardLogger(save_dir="weight_logs", name="kappa"),
