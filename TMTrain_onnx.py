...
from iwpc.models.layers import ConstantScaleLayer, RunningNormLayer, LambdaLayer
...
class SimpleMLP(nn.Module):
    def __init__(self, in_dim=2, hidden_dims=None, out_dim=1):
        super().__init__()
        if hidden_dims is None:
            hidden_dims = [128, 64, 64, 64, 64]
...
class KappaLightning(L.LightningModule, Visualisable):
    def __init__(self, lr: float = 1e-3):
        ...
        self.model = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(64, 1),
            LambdaLayer(lambda x: x.reshape((-1, 1))),
            target_encoding,
        )
