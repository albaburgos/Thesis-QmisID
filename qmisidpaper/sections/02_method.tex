...
Introducing the rescaled variable \(\kappa(e) = \log\!\left(\epsilon(e)/(1-\epsilon(e))\right)\), the ratio becomes \(\lambda_{\mathrm{SC}}^{R}(e_1,e_2)/\lambda_{\mathrm{OC}}^{R}(e_1,e_2) = \left(e^{\kappa(e_1)}+e^{\kappa(e_2)}\right)/\left(1+e^{\kappa(e_1)+\kappa(e_2)}\right)\), which depends on \(e_1\) and \(e_2\) only through their individual rescaled charge-flip variables.
