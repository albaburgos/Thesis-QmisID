...
\subsection{Prior Art: A Log-Likelihood Test}
...
\subsection{New Approach: A KL Divergence Test}
To formulate the problem in a continuous probabilistic framework, we denote the truth-level rate of di-electron events by \(\lambda^{T}(e_1,e_2)\). Each electron undergoes independent charge misidentification with probability \(\epsilon(e)\), giving reconstructed rates \(\lambda^{R}(e_1,e_2)\) of opposite-charge (OC) and same-charge (SC) events.

\begin{equation}
\begin{aligned}
    \lambda_{\mathrm{OC}}^{R}(e_1,e_2)
    &= \lambda^{T}(e_1,e_2)\,
       \Big[(1-\epsilon(e_1))(1-\epsilon(e_2))
       + \epsilon(e_1)\epsilon(e_2)\Big], \\[6pt]
    \lambda_{\mathrm{SC}}^{R}(e_1,e_2)
    &= \lambda^{T}(e_1,e_2)\,
       \Big[\epsilon(e_1)(1-\epsilon(e_2))
       + (1-\epsilon(e_1))\epsilon(e_2)\Big] \\[4pt]
    &= \lambda^{T}(e_1,e_2)\,
       \Big[\epsilon(e_1) + \epsilon(e_2)
       - 2\,\epsilon(e_1)\epsilon(e_2)\Big].
\end{aligned}
\end{equation}

The ratio of the expected number of SC and OC events for a given pair of electron kinematics \((e_1,e_2)\) is therefore a simple function of the corresponding charge-flip probabilities \(\epsilon(e_1)\) and \(\epsilon(e_2)\). The common factor \(\lambda^{T}(e_1,e_2)\) cancels, so the ratio \(\dfrac{\lambda_{\mathrm{SC}}^{R}(e_1,e_2)}{\lambda_{\mathrm{OC}}^{R}(e_1,e_2)}\) is independent of the unknown truth-level distribution.

Introducing the rescaled variable
\begin{equation}
\label{eq:kappa}
\kappa(e) \;=\; \log\!\left(\frac{\epsilon(e)}{1-\epsilon(e)}\right),
\end{equation}
this becomes
\begin{equation}
\label{eq:ratio_kappa}
\frac{\lambda_{\mathrm{SC}}^{R}(e_1,e_2)}{\lambda_{\mathrm{OC}}^{R}(e_1,e_2)}
\;=\;
\frac{e^{\kappa(e_1)}+e^{\kappa(e_2)}}{1+e^{\kappa(e_1)+\kappa(e_2)}}
\;=\;
\frac{\cosh\!\left(\dfrac{\kappa(e_1)-\kappa(e_2)}{2}\right)}{\cosh\!\left(\dfrac{\kappa(e_1)+\kappa(e_2)}{2}\right)},
\end{equation}
which depends on \(e_1\) and \(e_2\) only through the sum and difference of their rescaled charge-flip variables.
...
\)
Let \(q_i\) = \(\ell_i \in \{0,1\}\) denote the observed label for event \(i\), where \(\ell_i=1\) corresponds to an SC event and \(\ell_i=0\) to an OC event. 

A model distribution \(p\) most closely matches the data distribution \(q\) when the KL divergence or equivalently the cross-entropy loss $H(\frac{q}{p})$ is minimized. A binary cross-entropy loss can be written as shown below.
...
p 
=
\sigma\!\big(f(e_1^i,e_2^i)\big),
\) where \(f(e_1,e_2)\) is a real-valued classifier score.
\(
\sigma(f)
=
\frac{1}{1+e^{-f}}
...
\label{eq:logratio}
\log\!\left(
    \frac{p(\mathrm{OC}\mid e_1,e_2)}{p(\mathrm{SC}\mid e_1,e_2)}
\right)
=
\log\!\left(1 + e^{\kappa(e_1) + \kappa(e_2)}\right)
-
\log\!\left(e^{\kappa(e_1)} + e^{\kappa(e_2)}\right)
\end{equation}
...
\begin{equation}
    \label{eq:f_phi}
    f(e_1,e_2)
    \;=\; \log\!\left(1 + e^{\phi(e_1) + \phi(e_2)}\right)
    \;-\; \log\!\left(e^{\phi(e_1)} + e^{\phi(e_2)}\right).
\end{equation}

A well-trained classifier \(f_{\mathrm{min}}\) converges to the
solution \(\phi(e) = \kappa(e)\).
...
\)
using five hidden layers and producing a single scalar output \(\phi(e)\). This output is inverted to recover the corresponding charge-flip probability  \(\epsilon(e)\). Despite the simple architecture, the network is sufficiently expressive to model the smooth kinematic dependence of electron misidentification rates across detector phase space.
