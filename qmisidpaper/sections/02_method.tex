...
\section{Prior Art: A Log-Likelihood Test}
...
\section{Modern Approach: A KL Divergence Test}
...
\begin{figure}[H]
    ...
    \caption{
    Schematic illustration of the charge-misidentification process, mapping the truth-level distribution \(\lambda^{T}(e_1,e_2)\) to the reconstructed distribution \(\lambda^{R}(e_1,e_2)\).
    }
...
A model distribution \(p\) most closely matches the data distribution \(q\) when the KL divergence or equivalently the cross-entropy loss $H(q/p)$ is minimized. A binary cross-entropy loss can be written as shown below.
...
\) where \(f(e_1,e_2)\) is a real-valued classifier score wrapped by the sigmoid function
\(
\sigma(f)
=
\frac{1}{1+e^{-f}}
...
\begin{equation}
    \label{eq:classifier_ratio}
    \frac{p(\mathrm{SC}\mid e_1,e_2)}{p(\mathrm{OC}\mid e_1,e_2)}
    \;=\; \frac{\sigma(f)}{1 - \sigma(f)}
    \;=\; e^{f(e_1,e_2)}.
\end{equation}
