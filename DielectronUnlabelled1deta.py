# 1D Eta dielectron training, toy model 
...
class DielectronKernelTrainer(FDivergenceMinimizingKernelTrainer):
    ...
    def setup(self, stage: str) -> None:
        ...
        for batch in self.trainer.datamodule.val_dataloader():
            ...
            p_mask = labels == 0
            if p_mask.any():
                buf.append(samples[p_mask, :6].numpy())
    ...
    def _log_flip_plot(self, batch, global_step: int) -> None:
        ...
        for ax, oc_vals, sc_vals, bins, xlabel in [
            ...
            ( axes, oc[:, 0], sc_full[:, 1], np.linspace(-2.5, 2.5,   60), 'l1_eta'),
        ]:
...
if __name__ == "__main__":
    ...
    dm = PandasDirDataModule(
        ...
        weight_col="weight",
        split=0.8,
        dataloader_kwargs={"num_workers": 0, "batch_size": 2**17},
        # use_in_memory_dataset=True,
    )

    single_electron_encoding = TrivialEncoding(1)
    ...
    single_electron_flip_kernel = FiniteKernel(3, TrivialEncoding(1), init_log_probs=[np.log(0.998), np.log(0.001), np.log(0.001)])
    single_electron_one_flip_kernel = single_electron_flip_kernel.cut(lambda x: (x > 0))
    single_electron_eta_kernel = ConstantKernel([0], TrivialEncoding(1))
    model = basic_model_factory(NopeEncoding(1)&TrivialEncoding(1))

    print(single_electron_one_flip_kernel.cond_dimension, single_electron_one_flip_kernel.sample_dimension, single_electron_one_flip_kernel.cond_dimension)
    full_kernel = single_electron_eta_kernel | event_flip_kernel.cut(lambda x: (x.sum(dim=-1) > 0) and (x[0] == 0 or x[1] == 0))
    ...
    # unlabelled = DielectronKernelTrainer.load_from_checkpoint(
        ...
        sampled_kernel=single_electron_eta_kernel,
