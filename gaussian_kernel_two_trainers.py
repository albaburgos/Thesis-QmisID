# Experimentation: gaussian training classifier + kernel simultaneously (Chapter 5 Thesis)
...
if __name__ == "__main__":
    ...
    kernel = GaussianKernel(eta_pt_encoding)

    dm_classifier = PandasDirDataModule(
        dataset_dir=path,
        feature_cols=["truth_el_pt", "truth_el_eta"],
        target_cols=["label"],
        #weight_col="weight",
    ...
    train_loader_classifier = dm_classifier.train_dataloader()
    val_loader_classifier = dm_classifier.val_dataloader()
    lit = KappaLightning(lr=1e-3)
    ...
    trainer = L.Trainer(
        ...
        logger=TensorBoardLogger(save_dir="gaussian_logs", name="kappa"),
        num_sanity_val_steps=0,
    )

    trainer.fit(kernel, train_dataloaders=train_loader, val_dataloaders=val_loader)
    trainer.fit(lit, train_dataloaders=train_loader_classifier, val_dataloaders=val_loader_classifier)
    mixture = DiscreteBranchingKernel(eta_pt_encoding, DiscreteKernel(2, eta_pt_encoding, lit), GaussianKernel(eta_pt_encoding), explicit_mixture_label=True) 
    dielectron_kernel = kernel.__and__(kernel)

    DiscreteKernel(2, eta_pt_encoding, lit) & GaussianKernel(eta_pt_encoding)