...
sess = ort.InferenceSession("qmisid_data.onnx", providers=["CPUExecutionProvider"])
input_name = sess.get_inputs()[0].name
output_name = sess.get_outputs()[0].name
onnx_outputs_logits = True
...
def evaluate_for_visualiser_onnx(z: np.ndarray) -> np.ndarray:
    z_proc = np.stack(
        [
            z[:, 0] / z[:, 1],
            z[:, 2],
    ...
    out = sess.run(None, {input_name: z_proc})[0]
    if onnx_outputs_logits:
        out = sigmoid(out)
    return out.reshape(-1)


p_grid = evaluate_for_visualiser_onnx(z_input)
...
files = [
    "/Users/albaburgosmondejar/Desktop/Input/hhml_v2_2lSC_QMisID_data.root",
]
...
mask1 = (branches["same_charge"] == 1) & mask 
mask0 = (branches["same_charge"] == 0) & mask 
...
theta[0,0] = 0.000120776
theta[0,1] = 0.000656211
theta[0,2] = 0.00113747
theta[1,0] = 0.000249588
theta[1,1] = 0.00117526
theta[1,2] = 0.00256211
theta[2,0] = 0.00178397
theta[2,1] = 0.00824724
theta[2,2] = 0.0134212
theta[3,0] = 0.00513467
theta[3,1] = 0.0242338
theta[3,2] = 0.054148
...
#weights = np.ones(len(branches["l1_pt"]))
...
var_specs = [
    ...
    ("m_l1l2", lambda b: np.asarray(b["m_l1l2"], float) / 1e3, r"$m_{\ell_1\ell_2}$ [GeV]", 80, 100),
]
...
def binned_counts_and_errors(x, bins, xmin, xmax, weights=None):
    ...
    if weights is None:
        sumw2, _ = np.histogram(x, bins=bins, range=(xmin, xmax))
        errs = np.sqrt(sumw2)
    else:
        sumw2, _ = np.histogram(x, bins=bins, range=(xmin, xmax), weights=weights**2)
        errs = np.sqrt(sumw2)
